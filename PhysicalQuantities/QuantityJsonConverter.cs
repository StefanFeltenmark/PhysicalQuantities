...
namespace PhysicalQuantities
{
    ...
    public class QuantityJsonConverter : JsonConverter
    {
        ...
        public override void WriteJson(JsonWriter writer, object? value, JsonSerializer serializer)
        {
            ...
            if (q.Unit is Currency currency)
            {
                // Monetary: store SI value (EUR) + currency code
                writer.WritePropertyName("v");
                writer.WriteValue(q.ValueInSIUnits);
                writer.WritePropertyName("c");
                writer.WriteValue(currency.Name);
            }
            else
            {
                ...
                if (unitName != null)
                {
                    ...
                    if (q.PrefixIndex != Unit.SI_Prefix.unity)
                    {
            ...
            JsonReader reader, Type objectType, object? existingValue, JsonSerializer serializer)
        {
            ...
            {
                // Monetary format
                double euroAmount = jo["v"]!.Value<double>();
                string code = jo["c"]!.Value<string>()!;
                Currency? currency = FindCurrencyByCode(code);
                MonetaryAmount result = new MonetaryAmount(0, currency ?? Currencies.Euro);
                result.ValueInSIUnits = euroAmount;
                return result;
            }
            else if (jo["u"] != null)
            {
                ...
                var prefixIndex = (Unit.SI_Prefix)(jo["p"]?.Value<int>() ?? (int)Unit.SI_Prefix.unity);
            ...
            else
            {
                ...
                if (jo["d"] is JArray dims && dims.Count == 7)
                {
                    ...
                    int fp = jo["fp"]?.Value<int>() ?? (int)Unit.SI_Prefix.unity;
        ...
        {
            foreach (System.Reflection.FieldInfo field in typeof(Currencies).GetFields(
                System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static))
            {
                if (field.GetValue(null) is Currency c && c.Name == code)
                    return c;
            }
            return null;
        }
